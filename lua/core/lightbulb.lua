...
local function render(bufnr, client)
    ...
    local diagnostics =
        vim.lsp.diagnostic.from(vim.diagnostic.get(bufnr, { lnum = line }))

    ---@type lsp.CodeActionParams
    local params =
        vim.lsp.util.make_range_params(winnr, client.offset_encoding)
    params.context = {
...
M.attach_lightbulb = function(bufnr, client)
    ...
    if
        pcall(
            ...
            { group = buf_group_name, buffer = bufnr }
        )
    then
        return
    end
    ...
    local lb_buf_group =
        vim.api.nvim_create_augroup(buf_group_name, { clear = true })
